...
# Evaluate true masks vs predicted masks at provided IoU threshold. 
def eval(true_mask, pred_mask, iou_thres=0.5, metadata=None, print_metrics=False):
    ...
    res = {
        ...
        'FN': num_true_cells - tp,
        'IoUs': IoUs,
        'rightly_segmented_pixels': np.sum((true_mask != 0) & (pred_mask != 0)),
        'wrongly_segmented_pixels': np.sum((true_mask != 0) & (pred_mask == 0)),
        'rightly_not_segmented_pixels': np.sum((true_mask == 0) & (pred_mask == 0))
    }
    res.update(metadata) if metadata else None
...
# For metrics computed over multiple batches, return the totals and averages
def compute_metrics(results_2d, decimals=2, print_metrics=True):
    tp = sum([img_res['TP'] for img_res in results_2d])
    fp = sum([img_res['FP'] for img_res in results_2d])
    fn = sum([img_res['FN'] for img_res in results_2d])
    ...
    res_dict = OrderedDict([('imgs', images), ('cells', cells), ('TP', tp), 
                ('FP', fp), ('FN', fn), ('precision', precision), ('recall', recall), ('aP', aP) 
                # ('omission_ratio', avg_omission_ratio), ('oversegmentation_ratio', avg_oversegmentation_ratio)
                ])
...
def compute_grouped_metrics(results_2d, decimals=2, print_metrics=True):
    stratified_results = {}
    for result in results_2d:
        if not result['group'] in stratified_results:
            stratified_results[result['group']] = []
        stratified_results[result['group']].append(result)
    stratified_metrics = {group: compute_metrics(results, print_metrics=print_metrics) for group, results in stratified_results.items()}
    stratified_metrics.update(compute_metrics(results_2d, print_metrics=print_metrics))
...
class Inferenceinator():
    def __init__(self, loader, model_output_parser, IoU_threshold=0.5, processes=12):
        self.loader = loader
    ...
    def __call__(self, model, store_folder=None):
        ...
        with torch.no_grad():
            ...
            for batch in tqdm(self.loader, leave=False):
                ...
                results += self.eval_batch(batch, pred_masks_2d)
                           
        combined_metrics = compute_grouped_metrics(results, print_metrics=False)

        return combined_metrics

    def eval_batch(self, batch, pred_masks_2d):
        
        # Create a pool of worker processes
        with multiprocessing.Pool(processes=self.processes) as pool: 
            # Submit the processing function for each mask in the batch
            batch_results = [pool.apply_async(eval, args=(batch['masks_2d'][i].cpu().numpy(), pred_masks_2d[i], self.IoU_threshold, {'group': batch['groups'][i]})) for i in range(len(batch['masks_2d']))]
