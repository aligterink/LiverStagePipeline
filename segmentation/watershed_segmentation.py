...
def segment(image, resize=None, old_intensity_range=None):
    ...
    image = area_closing(image, area_threshold=1000)


    # apply threshold
    thresh = threshold_otsu(image, nbins=1000) #max(threshold_otsu(image), 0.01)
    bw = image > thresh #closing(image > thresh, square(5))
...
# This method is called when multithreading. Reads an image, performs watershed, then saves the image.
def segment_and_save(args):
    tif_path, seg_path, channel, resize_shape, intensity_range, eraser_path = args
    image = imageio.mimread(tif_path)[channel] # read image

    if eraser_path:
        eraser_mask = imageio.imread(eraser_path)
...
def segment_dir(tif_dir, seg_dir, threads, channel=1, resize_shape=None, normalize=True, eraser_dir=None):
    os.makedirs(seg_dir, exist_ok=True)

    if eraser_dir:
        tif_paths, eraser_paths = data_utils.get_two_sets(tif_dir, eraser_dir, common_subset=True, extension_dir1='.tif', extension_dir2='.png')
    else:
        tif_paths = data_utils.get_paths(tif_dir, extension='.tif', recursive=True)
        eraser_paths = [None] * len(tif_paths)

    seg_paths = [os.path.join(seg_dir, Path(tif_path).stem + '.tif') for tif_path in tif_paths]
    ...
    filewise_arguments = [(tif_path, seg_path, channel, resize_shape, file_range, eraser_path) for tif_path, seg_path, file_range, eraser_path in zip(tif_paths, seg_paths, file_ranges, eraser_paths)]
...
if __name__ == "__main__":
    ...
    # DAPI segmentation testing
    tif_path = '/mnt/DATA1/anton/data/lowres_dataset/images/NF135/D5/2019006c_D5_NF135_exp2_hsp_series_14_TileScan_001.tif'
    channel = 0
    resize_shape = None
    ...
    seg = segment(image, resize=resize_shape, old_intensity_range=intensity_range)
    print(type(seg))
    seg_img = seg #Image.fromarray(seg.astype(np.uint8), 'L')

    import matplotlib.pyplot as plt
    fig, axs = plt.subplots(nrows=1, ncols=2, sharex=True, sharey=True, figsize=(32, 16))
